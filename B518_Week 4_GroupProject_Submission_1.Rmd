...
```{r echo=FALSE, dataset_one_import, results="hide"}
...
Answering our first research question, the two way frequency (see appendix) shows that China had reported zero days exceeding a 50 new cases per million threshold. In contrast this threshold was crossed 94.% of days in Belgium, 89.9% in the USA and 84.9% in the UK. 
...
stopifnot("country" %in% names(d1_21_f))

tab_country <- as.data.frame(table(country = d1_21_f$country, useNA = "ifany"))
names(tab_country) <- c("country", "count")

tab_country$country <- ifelse(is.na(tab_country$country), "NA", as.character(tab_country$country))

# Add proportions
total_n <- sum(tab_country$count)
tab_country$proportion <- tab_country$count / total_n

# formatted table
knitr::kable(
  transform(tab_country,
            proportion = round(proportion, 3)),
  caption = "One-way table of Country: counts and proportions"
)
...
stopifnot(all(c("country","new_cases_per_million") %in% names(d1_21_f)))

if (!exists("threshold")) {
  threshold <- stats::quantile(d1_21_f$new_cases_per_million, 0.75, na.rm = TRUE)
}

d_tw <- subset(d1_21_f, !is.na(new_cases_per_million) & !is.na(country))

d_tw$high_day <- factor(d_tw$new_cases_per_million > threshold, levels = c(FALSE, TRUE),
                        labels = c("Low/Normal","High"))

# Two-way table
tw <- with(d_tw, table(country, high_day, useNA = "no"))

# Show raw counts
knitr::kable(as.data.frame.matrix(tw), caption = "Two-way table: Country × High-day indicator (counts)")

# Row proportions (within country)
row_prop <- prop.table(tw, margin = 1)
knitr::kable(round(as.data.frame.matrix(row_prop), 3),
             caption = "Row proportions: P(High/Low | Country)")

# Column proportions (within high/low)
col_prop <- prop.table(tw, margin = 2)
knitr::kable(round(as.data.frame.matrix(col_prop), 3),
             caption = "Column proportions: P(Country | High/Low)")

# Add margins (counts) for completeness
tw_margins <- addmargins(tw)
knitr::kable(as.data.frame.matrix(tw_margins),
             caption = "Two-way table with margins (counts)")
...
# checks if empty
if (length(x) == 0L) {
  cat("No non-missing values found for new_cases_per_million.\n")
} else {
  overall_stats <- c(
    mean   = mean(x),
    median = median(x),
  ...
  knitr::kable(as.data.frame(t(round(overall_stats[c("median","IQR","sd")], 1))),
               caption = "Overall center & spread (new_cases_per_million)")
}
...
# Combine
cn <- union(names(by_med), union(names(by_iqr), names(by_sd)))
by_summ <- data.frame(
  country = cn,
  median  = round(by_med[cn], 1),
  IQR     = round(by_iqr[cn], 1),
  sd      = round(by_sd[cn], 1),
  row.names = NULL
)

by_summ$median[is.na(by_summ$median)] <- NA
by_summ$IQR[is.na(by_summ$IQR)]       <- NA
by_summ$sd[is.na(by_summ$sd)]         <- NA

by_summ <- by_summ[order(by_summ$median, decreasing = TRUE, na.last = TRUE), ]

knitr::kable(by_summ, caption = "By-country center & spread (new_cases_per_million)")
...
heart_url <- "https://drive.google.com/uc?export=download&id=19rojU9JWTSA4eAcHq0hOFmJZWwZqanLx"

# Load dataset
hd <- readr::read_csv(heart_url, show_col_types = FALSE)

names(hd) <- make.names(names(hd))
...
```{r, results='hide'}
# Table A1: Continuous-variable summaries
continuous_vars <- c(
  "Age", "Cholesterol", "Heart.Rate", "BMI", "Triglycerides",
  "Exercise.Hours.Per.Week", "Stress.Level", "Sedentary.Hours.Per.Day",
  "Income", "Physical.Activity.Days.Per.Week", "Sleep.Hours.Per.Day"
)

cont_summary <- lapply(continuous_vars, function(v) {
  x <- hd[[v]]
...
cont_summary <- do.call(rbind, cont_summary)

knitr::kable(cont_summary, caption = "Appendix Table A1. Descriptive statistics for continuous variables.")

# Table A2: Categorical-variable counts (long format)
categorical_vars <- c(
...
cat_list <- lapply(categorical_vars, function(v) {
  if (!v %in% names(hd)) return(NULL)
  tt <- as.data.frame(table(Level = hd[[v]], useNA = "ifany"))
  tt$Variable <- v
  tt
})
cat_list <- Filter(Negate(is.null), cat_list)
cat_summary <- do.call(rbind, cat_list)
cat_summary$Level <- ifelse(is.na(cat_summary$Level), "NA", as.character(cat_summary$Level))

knitr::kable(cat_summary, caption = "Appendix Table A2. Frequency counts for categorical variables.")

# Appendix Figures

ggplot(hd, aes(x = BMI, fill = factor(Heart.Attack.Risk))) +
  geom_histogram(position = "dodge", bins = 30, alpha = 0.7) +
  labs(x = "BMI (kg/m²)", y = "Count", fill = "Heart Attack Risk") +
  theme_minimal()

ggplot(hd, aes(x = Age, fill = factor(Heart.Attack.Risk))) +
  geom_histogram(position = "dodge", bins = 30, alpha = 0.7) +
  labs(x = "Age (years)", y = "Count", fill = "Heart Attack Risk") +
  theme_minimal()
...
ggplot(hd, aes(x = Sex, fill = factor(Heart.Attack.Risk))) +
  ...
  labs(x = "Sex", y = "Proportion", fill = "Heart Attack Risk (0=No, 1=Yes)") +
  theme_minimal()
...
ggplot(hd, aes(x = factor(Smoking), fill = factor(Heart.Attack.Risk))) +
  ...
  geom_text(stat = "count", aes(label = ..count..),
            position = position_fill(vjust = 0.5), color = "black") +
  labs(x = "Smoking (0=No, 1=Yes)", y = "Proportion", fill = "Heart Attack Risk") +
  theme_minimal()
...
ggplot(hd, aes(x = factor(Obesity), fill = factor(Heart.Attack.Risk))) +
  ...
  geom_text(stat = "count", aes(label = ..count..),
            position = position_fill(vjust = 0.5), color = "black") +
  labs(x = "Obesity (0=No, 1=Yes)", y = "Proportion", fill = "Heart Attack Risk") +
  theme_minimal()
